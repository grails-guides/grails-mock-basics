We are going to use a unit test the dynamic finder with the help of `HibernateSpec`. It allows Hibernate to be used in Grails unit tests. It uses a H2 in-memory database.
