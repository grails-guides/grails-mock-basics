...
----
include::{sourceDir}/src/test/groovy/grails/mock/basics/ClassroomServiceSpec.groovy[tags=testEmailMock]
----
...
<1> Instantiate the mock implementation of `EmailService`.
<2> Inject the mock into the service.
<3> Invoke the service under test, which now has a mock implementation of `EmailService`.
<4> Use the spock syntax to verify invocation, params, and return a value.